...
g = 9.81; rho = 650; d = 10; x = [-d/2,d/2]; a = 0.25; a1 = 0; a2 = 0; a3 = 0 ; E = 10^5; Izz = 1; n = 20;
a4 = g*rho*a^2*d/(E*Izz);
f = @ (x) -g*rho*a^2*1.^(x); % rozložení působící síly %
L = E*Izz*diffmat([n n+4],4,x); %diferenciální rovnice%
T1 = diffrow(n+4,0,-d/2,x); T2 = diffrow(n+4,2,d/2,x); T3 = diffrow(n+4,1,-d/2,x); T4 = diffrow(n+4,3,d/2,x); % rovnice příslušné poč. podmínkám%
